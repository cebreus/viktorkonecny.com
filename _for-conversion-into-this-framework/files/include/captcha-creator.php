...
class FGCaptchaCreator extends FG_CaptchaHandler
{
    ...
    function GetError()
    {
        return 'Zadaný kód nesouhlasí. Opište prosím kód z obrázku';
    }
    ...
    function Create()
    {
        ...
        imagecolorallocate($this->image, 12, 12, 15 );
    ...
    function DrawChars($code)
    {
        ...
        for ($i = 0; $i < $N; $i++)
        {
            ...
            $bg = imagecolorallocate($image_char, 12, 12, 15 );
            ImageColorTransparent($image_char,$bg);

            $coloridx = rand(100, 150);
   ...
   function Blur($im)
   {
      ...
      $bg = ImageColorAllocate($imgTmp,12,12,15);
      ImageColorTransparent($imgTmp,$bg);
      ImageFill($imgTmp,0,0,$bg);
      $d = 1;

      ImageCopyMerge($imgTmp, $im, 0, 0, 0, $d, $width, $height-$d, 100);
      ImageCopyMerge($im, $imgTmp, 0, 0, $d, 0, $width-$d, $height, 100);
      ImageCopyMerge($imgTmp, $im, 0, $d, 0, 0, $width, $height, 100);
      ImageCopyMerge($im, $imgTmp, $d, 0, 0, 0, $width, $height, 100);
   ...
   function Distort($im)
   {
      ...
      $bg = ImageColorAllocate($image_out,12,12,15);
